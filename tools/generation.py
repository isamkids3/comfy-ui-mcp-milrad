...
from tools.document import validate_path_in_workspace
...
class CompositionElement(BaseModel):
    ...
    bbox: Optional[List[int]] = Field(
        None,
        description="Optional bounding box [y1, x1, y2, x2] normalized to 0-1000. y1 < y2, x1 < x2. Top-left origin. Only use if positioning is critical."
    )
    desc: str = Field(
        ...,
...
def register_workflow_generation_tools(
    ...
    @mcp.tool()
    def get_image_layout_guidelines() -> str:
        """Returns the detailed JSON formatting guidelines for generating images.
        Call this ONLY when you are preparing to generate an image and need to structure the prompt parameter.
        """
        import json
        try:
            meta_path = Path(workflow_manager.workflows_dir) / "text-image.meta.json"
            if meta_path.exists():
                with open(meta_path, "r", encoding="utf-8") as f:
                    meta = json.load(f)
                    return meta.get("prompt_guidelines", "Guidelines not found in metadata.")
            return "Guidelines metadata file not found."
    ...
    def _register_workflow_tool(definition: WorkflowToolDefinition):
        def _tool_impl(*args, **kwargs):
            ...
            # Keep original prompt for metadata registry
            original_prompt = kwargs.get("prompt")
            
            # If prompt or other parameters are Pydantic models or dicts, serialize them to minified JSON string
            for k in list(kwargs.keys()):
                v = kwargs[k]
                if isinstance(v, (dict, list)):
                    kwargs[k] = json.dumps(v, ensure_ascii=False)
                elif hasattr(v, "model_dump_json"):
                    kwargs[k] = v.model_dump_json(by_alias=True)
                elif hasattr(v, "json"):
                    kwargs[k] = v.json()
            ...
            # Pre-flight Path Resolution & Auto-upload for path/image arguments (Directive 2.3)
            for k in list(bound.arguments.keys()):
                v = bound.arguments[k]
                if isinstance(v, str):
                    # Check if it looks like an absolute path or virtual path (translated by client)
                    # Exclude prompts and JSON strings to avoid false-positive path validation errors
                    is_path_like = (
                        (v.startswith("/") or "\\" in v or k.endswith("_path") or "image" in k)
                        and k not in ("prompt", "negative_prompt")
                        and not (v.strip().startswith("{") or v.strip().startswith("["))
                    )
                    if is_path_like:
                        try:
                            # Validate security bounds
                            resolved_path = validate_path_in_workspace(v)
                            if os.path.isfile(resolved_path):
                                ext = os.path.splitext(resolved_path)[1].lower()
                                if ext in (".png", ".jpg", ".jpeg", ".webp", ".bmp", ".gif"):
                                    # Upload image bytes to ComfyUI input folder
                                    with open(resolved_path, "rb") as f:
                                        img_bytes = f.read()
                                    filename = os.path.basename(resolved_path)
                                    upload_res = comfyui_client.upload_image(
                            ...
                            logger.warning(f"Error handling path parameter '{k}' with value '{v}': {e}")
            ...
            try:
                ...
                prompt_meta = original_prompt
                if hasattr(original_prompt, "model_dump"):
                    prompt_meta = original_prompt.model_dump()
                elif hasattr(original_prompt, "dict"):
                    prompt_meta = original_prompt.dict()
        ...
        for param in definition.parameters.values():
            ...
            if param.annotation in (int, float):
                ...
                annotation_type = Any
            elif param.name == "prompt" and definition.tool_name in ("text_image", "image_reference_and_text_to_image"):
                annotation_type = IdeogramPrompt
            else:
            ...
            if param.required:
                ...
                annotations[param.name] = annotation_type
            else:
                ...
                annotations[param.name] = final_annotation
        ...
        # Add session_id as optional parameter
        optional_params.append(inspect.Parameter(
            name="session_id",
            kind=inspect.Parameter.POSITIONAL_OR_KEYWORD,
        ...
        annotations["session_id"] = Optional[str]
...
def _update_workflow_params(workflow: dict, param_overrides: dict) -> dict:
    ...
    for param_name, override_value in param_overrides.items():
        ...
        # Serialize structured prompt/override values into a single-line minified JSON string
        if param_name in ("prompt", "negative_prompt"):
            if isinstance(override_value, (dict, list)):
                override_value = json.dumps(override_value, ensure_ascii=False)
            elif hasattr(override_value, "model_dump_json"):
                override_value = override_value.model_dump_json(by_alias=True)
            elif hasattr(override_value, "json"):
                override_value = override_value.json()
