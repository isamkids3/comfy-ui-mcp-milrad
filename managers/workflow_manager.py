...
class WorkflowManager:
    ...
    def _load_workflows(self):
        ...
        for workflow_path in sorted(self.workflows_dir.glob("*.json")):
            ...
            if not isinstance(workflow, dict):
                logger.warning("Skipping workflow %s because it is not a JSON object", workflow_path.name)
                continue
            ...
            metadata = self._load_workflow_metadata(workflow_path)
            raw_tool_name = metadata.get("name", self._derive_tool_name(workflow_path.stem))
            tool_name = self._dedupe_tool_name(self._normalize_name(raw_tool_name))
            description = metadata.get("description", self._derive_description(workflow_path.stem))
    ...
    def render_workflow(self, definition: WorkflowToolDefinition, provided_params: Dict[str, Any], defaults_manager: Optional["DefaultsManager"] = None):
        ...
        # Extract aspect ratio from prompt JSON if not explicitly provided as a parameter
        aspect_ratio = provided_params.get("aspect_ratio")
        if not aspect_ratio and "prompt" in provided_params:
            prompt_val = provided_params["prompt"]
            prompt_dict = None
            if isinstance(prompt_val, dict):
                ...
                try:
                    ...
                    pass
            if isinstance(prompt_dict, dict) and "aspect_ratio" in prompt_dict:
                aspect_ratio = prompt_dict["aspect_ratio"]

        # Map aspect ratio to standard ComfyUI selector strings
        if aspect_ratio:
            aspect_ratio_str = str(aspect_ratio).strip()
            ASPECT_RATIO_MAP = {
                "1:1": "1:1 (Square)",
            ...
            if aspect_ratio_str in ASPECT_RATIO_MAP:
                aspect_ratio = ASPECT_RATIO_MAP[aspect_ratio_str]
        ...
        for param in definition.parameters.values():
            ...
            if param.name == "aspect_ratio" and aspect_ratio:
