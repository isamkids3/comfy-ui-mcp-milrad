...
* run the dynamic `text_image` workflow tool with your prompt (or a default)
...
* `text_image`
* `image_image`
* `image_reference_and_text_to_image`
* `read_user_document`
* `view_image`
* `regenerate`
* `get_job`
...
- call `text_image` with just a prompt (server defaults handle the rest)
...
### Generation Tools (Dynamic Workflows)

- **`text_image`**: Generate images from a text prompt (rendered via Ideogram 4.0).
- **`image_image`**: Modify/translate an image via Img2Img (requires `prompt` and `init_image`).
- **`image_reference_and_text_to_image`**: Generate a new image using a reference image structure guide (requires `prompt` and `reference_image`).
- **`regenerate`**: Regenerate an existing asset with optional parameter overrides (requires `asset_id`).

### Document Ingestion Tools

- **`read_user_document`**: Parses and extracts text content from local user uploads (`.txt`, `.pdf`, or `.docx`) inside the secure workspace bounds.
...
- Calls `text_image(prompt="a hero image for a website")` → gets `asset_id`
...
- Calls `text_image(prompt="a modern logo")` → gets `asset_id`
...
├── async_comfyui_client.py# Async ComfyUI websocket client
...
│   ├── generation.py      # Dynamic workflow tools & auto-uploader
│   ├── asset.py
...
│   ├── upload.py
│   ├── publish.py
│   └── document.py        # Secure document reader tool
├── models/                # Data models
│   ├── workflow.py
│   └── asset.py
└── workflows/             # Workflow JSON files
    ├── text-image.json
    ├── image-image.json
    └── image_reference_and_text_to_image.json
```
...
To prevent path traversal attacks and ensure absolute file access is secure, the server enforces workspace nesting constraints:
- **`SHARED_WORKSPACE_ROOT`**: Define this environment variable to specify the root directory of the secure workspace (defaults to `/Users/adamdali/Documents/MilleniumRadius/gen-content`).
- **Nesting Validation**: Tools that accept local paths (such as `read_user_document` or dynamic workflows accepting local image paths) will automatically resolve the paths and verify that they are strictly nested inside `SHARED_WORKSPACE_ROOT`. Any attempt to read or reference files outside this root raises a security error.
- **Pre-Flight Path Parameter Upload**: Dynamic tools automatically detect if string parameters represent image files. If an image path is valid and within the workspace root, the server automatically reads the file bytes, uploads it to the ComfyUI server, and updates the workflow parameter with the uploaded filename.
...
- Built for the MilleniumRadius creative generation pipeline.
