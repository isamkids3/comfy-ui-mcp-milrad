...
class ComfyUIClient:
    ...
    def _wait_for_prompt(self, prompt_id: str, max_attempts: int = 120):
    ...
    def upload_image(
        ...
        Uses ComfyUI's /upload/image endpoint to store an image file that can
        be referenced by LoadImage nodes in workflows.
        ...
        Returns:
            Dict with keys: 'name' (stored filename), 'subfolder', 'type' ("input").

        Raises:
            Exception: If the upload fails or ComfyUI returns an error.
        """
        try:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"ComfyUI upload failed: {response.status_code} - {response.text}"
                )

            result = response.json()
            logger.info(
                "Uploaded image to ComfyUI: %s (subfolder=%s)",
                result.get("name", filename),
