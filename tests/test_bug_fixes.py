...
class TestOverrideMismatchReporting:
    ...
    def test_aspect_ratio_resolution_mapping(self, tmp_path):
        """Test that aspect_ratio is extracted from prompt JSON and mapped to full ComfyUI selector strings."""
        wf_dir = tmp_path / "workflows"
        wf_dir.mkdir()
        ...
        wf = {
            "37": {
                "inputs": {
                    "aspect_ratio": "PARAM_ASPECT_RATIO"
                },
        ...
        # Case 1: aspect_ratio passed as direct parameter with short key
        rendered1 = mgr.render_workflow(defn, {"aspect_ratio": "16:9"})
        assert rendered1["37"]["inputs"]["aspect_ratio"] == "16:9 (Widescreen)"

        # Case 2: aspect_ratio omitted but present in prompt JSON string
        prompt_json = json.dumps({"aspect_ratio": "9:16", "high_level_description": "test"})
        rendered2 = mgr.render_workflow(defn, {"prompt": prompt_json})
        assert rendered2["37"]["inputs"]["aspect_ratio"] == "9:16 (Portrait Widescreen)"

        # Case 3: aspect_ratio omitted but present in prompt dictionary
        rendered3 = mgr.render_workflow(defn, {"prompt": {"aspect_ratio": "1:1"}})
        assert rendered3["37"]["inputs"]["aspect_ratio"] == "1:1 (Square)"
